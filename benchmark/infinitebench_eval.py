    ...
    for item in pred_list:
        if item != "":
            try:
                ...
                print(item)
                print(item.strip("0"))
                pred_nums.append(int(item.strip("0")))
...
def compute_scores(preds_path, data_name: str):
...
def main(args):
    ...
    for fp in file_paths:
        ...
        if dataset_name not in ALL_TASKS:
        ...
        with open(fp, "r", encoding="utf-8") as f:
            for line in f:
                ...
                preds.append(data.get("pred", "NO PREDICTION"))
                answers.append(data.get("answers", "XXXXXXXXXX"))
        score = get_score(answers, preds, dataset_name)
