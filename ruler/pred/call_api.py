...
parser.add_argument("--sgl_config_path", type=str, default="")
parser.add_argument("--inf_llm_config_path", type=str, default="")
...
parser.add_argument("--use_inf_llm", action=argparse.BooleanOptionalAction, default=False)
...
def get_llm(tokens_to_generate, device="auto"):
    if args.server_type == 'trtllm':
        ...
        llm = HuggingFaceModel(
            ...
            use_inf_llm=args.use_inf_llm,
            inf_llm_config_path = args.inf_llm_config_path,
            device=device,
        )
    ...
    # elif args.server_type == 'sglang-ours':
    #     from model_wrappers import SGLModel
    #     llm = SGLModel(
    #         name_or_path=args.model_name_or_path,
    #         config_path=args.sgl_config_path,
    #         repetition_penalty=1,
    #         temperature=args.temperature,
    #         top_k=args.top_k,
    #         top_p=args.top_p,
...
def main():
    ...
    # if os.path.exists(args.save_dir / f'{args.task}.jsonl'):
    #     print(f'{args.task}.jsonl exists, continue') 
    #     return
    ...
    merged_pred_file = args.save_dir / f'{args.task}.jsonl'
    if os.path.exists(merged_pred_file) \
        and len(read_manifest(merged_pred_file)) == len(read_manifest(task_file)):
        print(f'{args.task}.jsonl exists, continue') 
        return
    ...
    if os.path.exists(pred_file):
        ...
        if len(read_manifest(task_file)) < len(pred_index):
    ...
    if args.server_type == 'hf':
        llms = [get_llm(config['tokens_to_generate'], device=f"cuda:{i}") for i in range(args.threads)]
    else:
        # Load api
    ...
    with open(pred_file, 'at', encoding="utf-8", buffering=1) as fout:
        ...
        for batch_idx, batch in tqdm(enumerate(batched_data), total=len(batched_data)):
            ...
            if args.server_type == 'hf':
                llm = llms[batch_idx % args.threads]
    ...
    timing_file = os.path.join(os.path.dirname(pred_file), "dataset_timing.json")
    if os.path.exists(timing_file):
        with open(timing_file, "r", encoding="utf-8") as f:
            dataset_timing = json.load(f)
    else:
        dataset_timing = {}
    if args.task in dataset_timing:
        dataset_timing[args.task] += (time.time() - start_time)
    else:
        dataset_timing[args.task] = time.time() - start_time
    with open(timing_file, "w", encoding="utf-8") as f:
        json.dump(dataset_timing, f)
